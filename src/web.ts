...
async checkPermissions(): Promise<{ status: string }> {
  ...
  async getMedias(options: {
    ...
    try {
      ...
      for await (const entry of dirHandle.values()) {
        if (entry.kind === "file") {
          ...
          if (file.type.startsWith("image/") || file.type.startsWith("video/")) {
            ...
            // Falls es ein Bild ist, Breite/Höhe und BaseColor auslesen
            if (file.type.startsWith("image/")) {
              await this.getImageDimensions(file).then(dimensions => {
                mediaItem.width = dimensions.width;
                mediaItem.height = dimensions.height;
              });

              if (options.includeBaseColor) {
                await this.getDominantColor(file).then(baseColor => {
      ...
      // Sortierung nach `createdAt` (newest = absteigend, oldest = aufsteigend)
      mediaArray.sort((a, b) => options.sort === "oldest" ? Number( a.createdAt) - Number(b.createdAt) : Number( b.createdAt) - Number( a.createdAt));
    ...
    return new Promise(resolve => {
      ...
      img.onload = () => {
        ...
        if (ctx) {
          ...
          resolve(`#${pixel[0].toString(16).padStart(2, '0')}${pixel[1].toString(16).padStart(2, '0')}${pixel[2].toString(16).padStart(2, '0')}`);
        } else {
          resolve("#000000"); // Fallback falls kein Kontext verfügbar ist
        }
      };
      img.src = URL.createObjectURL(file);
    });
